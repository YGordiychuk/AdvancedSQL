...
CREATE PROCEDURE Task2(@p_DbName NVARCHAR(30), @p_SchemaName NVARCHAR(30), @p_TableName NVARCHAR(30))
AS
BEGIN
...
-- cte and query for first part of date
WITH first_query AS(
	SELECT
		CASE
			WHEN  @p_TableName != '%' 
		THEN 
			...
			FROM ['+@p_DbName+'].[INFORMATION_SCHEMA].[TABLES] a
			JOIN ['+@p_DbName+'].[INFORMATION_SCHEMA].[COLUMNS] b 
...
-- cte and query for second part of date
WITH cte_2 as (
SELECT
...
--SECOND PART OF DATA only counters
SELECT
	@v_Query = STRING_AGG(CAST([query_text] AS nvarchar(MAX)), '') WITHIN GROUP (ORDER BY [query_text])
FROM secong_query;
INSERT INTO @result_2([Table_name], [Column_name], [dist_rows_cnt], [cnt_null], [cnt_all])
EXEC (@v_Query);
...
SELECT 
	...
	b.[dist_rows_cnt] as 'Count of DISTINCT values', 
...
--EXEC Task2 'TRN1', 'hr', '%'
--EXEC Task2 'AdventureWorksDW', 'dbo', 'DimDate'
